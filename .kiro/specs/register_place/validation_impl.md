...
- Task 4.1: バックエンドのバリデーションと重複防止を検証する
- Task 4.2: フロントエンドの入力と結果表示を検証する
...
- Task 4.1 も要件/設計どおりにテストで担保されており、完了と判断できる。
- Task 4.2 も要件/設計どおりにテストで担保されており、完了と判断できる。
...
### Task 4.1（バックエンド）
- モデルテスト: `backend/spec/models/place_spec.rb`
  - 必須項目の未入力を拒否することを検証
  - `tabelog.com` ドメイン制約を検証
  - URL正規化と正規化後の重複判定を検証
- リクエストテスト: `backend/spec/requests/api/places_spec.rb`
  - 登録成功の応答を検証
  - 必須項目未入力（店名/食べログURL）で422が返ることを検証
  - `tabelog.com` 以外のURLで422が返ることを検証
  - URL重複時に409 + `existing_place_id` が返ることを検証

### Task 4.2（フロント）
- 入力バリデーション/表示: `frontend/tests/App.test.tsx`
  - 必須項目未入力でエラーが表示されることを検証
  - `tabelog.com` 以外のURLで理由が表示されることを検証
  - 必須項目のみで登録でき、結果が表示されることを検証
- 登録APIの結果判定: `frontend/tests/api/places.test.ts`
  - 登録成功/重複/バリデーション/失敗/詳細取得失敗を検証
- 登録フロー: `frontend/tests/hooks/useRegisterPlace.test.tsx`
  - 登録成功でコールバック実行、重複時の案内表示を検証
- 結果表示: `frontend/tests/components/PlaceResult.test.tsx`
  - 登録結果の表示/取得失敗時メッセージを検証

## ギャップ/懸念
- なし（Task 4.2 を含む要件・設計に対する未実装は確認できず）。

## 次のアクション
- なし（register_place のタスクは完了）
